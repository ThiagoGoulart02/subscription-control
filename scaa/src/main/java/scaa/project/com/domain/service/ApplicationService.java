...
public class ApplicationService implements ApplicationRepositoryImpl {
    ...
    public ResponseEntity<ApplicationResponseDTO> createApplication(ApplicationDTO dto) {
        if (repository.findByName(dto.name()) != null) return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
    ...
    public ResponseEntity<ApplicationResponseDTO> updateApplication(Long id, ApplicationDTO dto) {
        ...
        if(app.isEmpty()) return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);

        app.get().setName(dto.name());
    ...
    public ResponseEntity<?> deleteApplication(Long id) {
        if(repository.findById(id).isEmpty()) return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
